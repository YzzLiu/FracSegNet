...
def fracture_seg(input_dicom_or_nii, output_file, model_folder_name, tta=False, clean_up=False, save_stl=0,
...
def main():
    ...
    for file in work_dir_list:
        ...
        RI_input_file = os.path.join(work_dir, file, file) + '_RI.nii.gz'
        RI_output_file = os.path.join(work_dir, file, file) + '_RI_mask.nii.gz'
        LI_input_file = os.path.join(work_dir, file, file) + '_LI.nii.gz'
        LI_output_file = os.path.join(work_dir, file, file) + '_LI_mask.nii.gz'
        SA_input_file = os.path.join(work_dir, file, file) + '_SA.nii.gz'
        SA_output_file = os.path.join(work_dir, file, file) + '_SA_mask.nii.gz'

        try:
            print('--------------------------Start Anatomical Segmentation--------------------------')
            flag = fracture_seg(RI_input_file, RI_output_file, model_path)
            flag = fracture_seg(LI_input_file, LI_output_file, model_path)
            flag = fracture_seg(SA_input_file, SA_output_file, model_path)
        except:
            print('error: ', file)
