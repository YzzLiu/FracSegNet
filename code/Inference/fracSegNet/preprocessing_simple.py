...
import numpy as np
from fracSegNet.configuration import RESAMPLING_SEPARATE_Z_ANISOTROPY_THRESHOLD
...
def resample_data_or_seg(data, new_shape, is_seg, axis=None, order=3, do_separate_z=False, cval=0, order_z=0):
    ...
    if False:
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                reshaped.append(
                    resize_fn(data[c], new_shape, order, cval=cval, **kwargs #  进进进
                 )[None])
